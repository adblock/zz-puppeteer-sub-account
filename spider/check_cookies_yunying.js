...
const updateCookieStatus = async (wangwang,status,cookie)=>{
    if(status===0){
        await G_MONGO.db.collection('sub_account_yunying_login').updateMany({
...
async function get_sub_account(){
    ...
    let sql = `
        ...
        and t_task.f_foreign_task_state_id = 2
    `;
...
(async () => {
    ...
    await asyncForEach(order_list, async (order, index) => {
        ...
        let account_list = await G_MONGO.db.collection('sub_account_yunying_login').find({'wangwang_id': wangwang}).toArray();
