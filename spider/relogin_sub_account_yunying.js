        ...
        await page.on('response', async (response) => {
            ...
            if (response.url().indexOf('https://login.taobao.com/newlogin/login.do') > -1) {
                ...
                if (data.content.data.titleMsg) {
                    ...
                    await saveErrorCookies(wangwang, data.content.data.titleMsg, account, password);
                    await browser.close()
                }
            }
...
async function get_sub_account(wangwang) {
    ...
    // let sql = "select f_lz_account, f_lz_password" +
    //     " from t_order join t_product on t_order.f_foreign_product_id = t_product.id " +
    //     "and t_order.f_copy_wangwangid='" + wangwang +
    //     "' and t_product.f_foreign_sku_kind in (\'直通车\', \'钻展\', \'超级推荐\', \'淘宝/天猫代运营\')" +
    //     " ORDER BY t_order.id DESC LIMIT 1;";
    let sql = `
...
const saveSuccessCookies = async (cookie, wangwang, account, password) => {
    ...
    await G_MONGO.db.collection('sub_account_yunying_login').deleteMany({
        'wangwang_id': wangwang
    });
    await G_MONGO.db.collection('sub_account_yunying_login').insertOne(cookieObj);
};
...
(async () => {
    ...
    let invalid_account = [];
    if (type === 'all') {
        invalid_account = await G_MONGO.db.collection('sub_account_yunying_login')
            .find({})
            .toArray();
    } else {
        invalid_account = await G_MONGO.db.collection('sub_account_yunying_login').find({
            'f_valid_status': 0,
            $or: [
                {'f_is_used': {}},
                {'f_is_used': 1},
