...
async function get_sub_account(wangwang) {
    ...
    let sql_boss = `select f_account as f_lz_account, f_password as f_lz_password from t_order_account where f_account like '%${wangwang}%'`;
    let accountList = await mysqlCfgSql(new_boss, sql_boss);
    //账号密码的去重
    if(accountList.length>1){
        let jsons = accountList.map(item => {
            item['f_lz_account'] = item['f_lz_account'].trim();
            item['f_lz_password'] = item['f_lz_password'].trim();
...
const saveSuccessCookies = async (cookie, wangwang, account, password) => {
    ...
    //更新数据
    await G_MONGO.db.collection('sub_account_login').updateMany({'account':account,'password': password,'f_use': {$exists: 1}},
        { $set: cookieObj});
...
(async () => {
    ...
    let arrList = Array.from(new Set(invalid_account.map(e => e['wangwang_id'])));

    await asyncForEach(arrList, async (invalid, index) => {
        let wangwang = invalid;
